...
function defaultGeometry(props: any = {}) {
  const { w = 1, h = 1, m = 0.76, n = 0.06, d = 0.6, shape = new THREE.Shape() } = props;
  [
    [0, 0],
    [0, h],
    [(w - m) / 2, h],
...
export class GeometryController {
  ...
  bind(key = "") {
    if (key === "file") return { onChange: this.change.bind(this) };
    if (key === "mesh") return { onClick: this.click.bind(this) };
  }
